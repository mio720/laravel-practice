                ...
                <div class="card-body">
                    <form action="{{ route('update', ['id'=>$product->id]) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="text" id="txtProductName" name="product_name" @if ($errors->has('product_name')) class="form-control is-invalid" @else class="form-control" @endif value="{{ old('product_name', $product->product_name) }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <select id="drpCompanyId" name="company_id" @if ($errors->has('company_id')) class="form-select is-invalid" @else class="form-select" @endif>
                                    ...
                                    <option value="{{ $company->id }}" @if ((int)old('company_id', $product->company_id) === $company->id) selected @endif >{{ $company->company_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                <input type="number" id="numPrice" name="price" @if ($errors->has('price')) class="form-control is-invalid" @else class="form-control" @endif value="{{ old('price', $product->price) }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <p class="col-sm-2 col-form-label">在庫数</p>
                            <div class="col-sm-10">
                                <input type="number" id="numStock" name="stock" @if ($errors->has('stock')) class="form-control is-invalid" @else class="form-control" @endif value="{{ old('stock', $product->stock) }}">
                            </div>
                        </div>
                        <div class="row mb-3">
                            <p class="col-sm-2 col-form-label">コメント</p>
                            <div class="col-sm-10">
                                <textarea id="areaComment" name="comment" class="form-control" style="height: 100px">{{ old('comment', $product->comment) }}</textarea>
                            </div>
                        </div>
